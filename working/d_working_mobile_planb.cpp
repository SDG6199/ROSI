...
int pole_tag,pole_tag_buffer=0;
int pole_tag_flag=1;
...
{
    ros::NodeHandlePtr n = boost::make_shared<ros::NodeHandle>();
    //횡단불가영역 침범 여부 토픽 수신하는 subscriber   @ 사실 detect_line에서 바로 신호등으로 buzzer_tag publish하면 된다.
    ...
    {   
        ...
        {
            /*
            if(횡단불가영역 침범 여부 토픽==1)   @ 사실 detect_line에서 바로 신호등으로 buzzer_tag publish하면 된다.
            {
                buzzer_tag.data=1;
            }
            */
...
void thread_pole_tag_polling()  //@
{
    ...
    while(ros::ok())
    {
        int now_pole_tag, prev_pole_tag;
        

        now_pole_tag=pole_tag_buffer;
        prev_pole_tag=now_pole_tag;
        while(prev_pole_tag==now_pole_tag)
        {
            now_pole_tag=pole_tag_buffer;
            direction_roi_pub.publish(direction_roi);
            loop_rate.sleep();
        }
        if((prev_pole_tag==1 && now_pole_tag==2) || (prev_pole_tag==2 && now_pole_tag==4))  
        {
            direction_roi.data=1;   //left
        }
    }
...
int main(int argc, char **argv)
{
    ...
    boost::thread t3(thread_pole_tag_polling);
    ...
    while(ros::ok())
    {
        ...
        */
        ...
        else
        {
            ...
            if(ros::Time::now()-under_sensor_time>ros::Duration(3))
            {
            ...
            if(is_car==true)
            {
                ...
                while(is_car_time-isnot_car_time<ros::Duration(2))
                {
