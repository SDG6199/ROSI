...
{   
    ...
    int path_index=0;
    int path_vec_size=path_vec.size();
    ...
    while(ros::ok())
    {
        ...
        {
            if(path_index==path_vec_size-1)
            {
                path_index=0;
            }
            pole_num_arr[0]=path_vec[path_index];
            printf("%d",path_vec[path_index]);
            pole_num_arr[1]=path_vec[path_index+1];

            int pole_rel=pole_num_arr[1]-pole_num_arr[0];
            switch(pole_rel)
            {
                case -1:    //left
                    pole_angle_arr[0]=1;
...
int main(int argc, char **argv)
{
    ...
    boost::thread t3(thread_pole_angle);
    boost::thread t4(thread_pole_num);
